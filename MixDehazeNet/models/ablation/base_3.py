...
class MixStructureBlock(nn.Module):
    def __init__(self, dim):
        ...
        self.conv3_3_1 = nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=dim)
        self.conv3_3_2 = nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=dim)
        self.conv3_3 = nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=dim)
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.conv2(x)
        x = torch.cat([self.conv3_3_1(x),self.conv3_3_2(x),self.conv3_3(x)],dim=1);
        x = self.mlp(x)
        x = identity + x
