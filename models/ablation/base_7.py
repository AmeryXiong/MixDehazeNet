...
class MixStructureBlock(nn.Module):
    def __init__(self, dim):
        ...
        self.conv3_7_1 = nn.Conv2d(dim, dim, kernel_size=3, padding=3, groups=dim, dilation=3, padding_mode='reflect')
        self.conv3_7_2 = nn.Conv2d(dim, dim, kernel_size=3, padding=3, groups=dim, dilation=3, padding_mode='reflect')
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.conv2(x)
        x = torch.cat([self.conv3_7_1(x),self.conv3_7_2(x),self.conv3_7(x)],dim=1);
        x = self.mlp(x)
        x = identity + x
