...
class MixStructureBlock(nn.Module):
    def __init__(self, dim):
        ...
        self.conv3_13_1 = nn.Conv2d(dim, dim, kernel_size=5, padding=6, groups=dim, dilation=3, padding_mode='reflect')
        self.conv3_13_2 = nn.Conv2d(dim, dim, kernel_size=5, padding=6, groups=dim, dilation=3, padding_mode='reflect')
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        x = self.conv2(x)
        x = torch.cat([self.conv3_13_1(x),self.conv3_13_2(x),self.conv3_13(x)],dim=1);
        x = self.mlp(x)
        x = identity + x
